...
function save() {
  ...
  if (instagram.error || youtube.error) return;
  ...
  <form class="profile-editor" @input="saved = false" @submit.prevent="save">
    <section class="editor-section">
      ...
      <div class="editor-grid">
        <DesignSystemFormField label="Nome de exibição"
          ><input v-model="form.name" required maxlength="70"
        ...
        <DesignSystemFormField
          ...
          ><div class="phone-field">
            <em>+55</em
            ><input v-model="form.whatsapp" required type="tel" /></div
        ></DesignSystemFormField>
        <DesignSystemFormField
          ...
          ><template #label
            ...
            <em>{{ form.headline.length }}/120</em></template
          ><input v-model="form.headline" required maxlength="120"
        /></DesignSystemFormField>
        <DesignSystemFormField
          class="editor-grid__full"
          ...
          ><template #label
            ...
            <em>{{ form.bio.length }}/500</em></template
          ><textarea v-model="form.bio" required maxlength="500" />
        </DesignSystemFormField>
        <DesignSystemFormField
      ...
      <div class="editor-grid">
        ...
        >
          <input
            ...
            :aria-invalid="socialErrors.instagram ? 'true' : 'false'"
            @input="clearSocialError('instagram')"
            @blur="validateSocialField('instagram')"
          />
        </DesignSystemFormField>
        <DesignSystemFormField
          label="YouTube"
        ...
        >
          <input
            ...
            :aria-invalid="socialErrors.youtube ? 'true' : 'false'"
            @input="clearSocialError('youtube')"
            @blur="validateSocialField('youtube')"
          />
        </DesignSystemFormField>
      </div>
    </section>
    ...
    <section class="editor-section">
      ...
      <DesignSystemFormField
        ...
        ><select v-model="form.primaryService">
    ...
    <section class="editor-section">
      ...
      <div v-if="!form.allJoinville" class="neighborhood-picker">
        <button
          ...
          @click="toggleNeighborhood(item.name)"
        >
          <UIcon
            :name="
    ...
    <div class="editor-savebar">
      <span
        ...
        />
        {{ saved ? "Alterações salvas" : "Há alterações não salvas" }}</span
      >
      <div>
        <small
          >Os campos acima representam as informações públicas do perfil.</small
        ><UButton type="submit" color="primary" :disabled="saved"
...
.editor-section header > div > span {
  display: grid;
  place-items: center;
  width: 34px;
  height: 34px;
...
.service-picker button.selected > svg {
  color: #397a69;
}
.primary-service {
  max-width: 320px;
