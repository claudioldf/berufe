...
| S037  | DONE   | WhatsApp handoffs redirect safely and count source-aware aggregate clicks.              |
...
- Add `search_events` with an optional unambiguous service/neighborhood dimension, Joinville city code, result count, profile-opened flag, optional search-handoff flag, and timestamps. Expression and structured fallback searches leave the retained query text null.
- Reject malformed request values but do not retain the raw or normalized expression, names, phone numbers, free-form notes, a customer account, a cookie identifier, or persistent visitor identity.
...
- Nuxt carries the sanitized request from results to a search-derived profile and WhatsApp handoff. Signed interaction tokens continue to contain identifiers only. Search events, aggregate metrics, and redirect logs do not store the request; the LLM analysis cache expires after 24 hours.
